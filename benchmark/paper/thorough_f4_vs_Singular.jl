...
function run_system_groebner(name, system)
    println("---------------\n$name")
    println("my")
...
function run_system_singular(name, system)
    println("---------------\n$name")
    println("singular")
...
function run_f4_ff_degrevlex_benchmarks(ground)
    ...
    ("cyclic 9", Groebner.cyclicn(9, ground=ground)), # 0.60 vs 0.9
        ...
        ("reimer 6"    ,Groebner.reimern(6, ground=ground)),  # 0.19  vs 0.36
        ("reimer 7"    ,Groebner.reimern(7, ground=ground)),  # 0.19  vs 0.36
        ("reimer 8"    ,Groebner.reimern(8, ground=ground)),  # 0.19  vs 0.36
    ]

    for (name, system) in systems
        task1 = @spawn run_system_my(name, system)
        task2 = @spawn run_system_singular(name, system)
        wait(task1)
        wait(task2)
    end
end
