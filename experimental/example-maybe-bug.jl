...
R, (x1, x2, x3) = polynomial_ring(GF(2^31 - 1), ["x1", "x2", "x3"], ordering=:degrevlex)

s = [x1 * x2^10 + 2, x1^11 * x3^3 + 3, x2^6 * x3 + 4 * x1^4 - 5]

gb1 = Groebner.groebner(s, loglevel=-3, linalg=:randomized, threaded=:no)
gb2 = Groebner.groebner(s, loglevel=-3, linalg=:randomized, threaded=:no)

m = Groebner.UseSpareBitsArithmeticZp(Primes.prevprime(UInt(p6), 33))

@code_native debuginfo = :none Groebner.dense_row_mod_p!(UInt[1, 2, 3], m)

s = Groebner.cyclicn(8, ordering=:degrevlex, ground=AbstractAlgebra.GF(p4))
