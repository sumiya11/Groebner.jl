...
"""
function fglm(basis::Basis, ht::MonomialHashtable)
    @assert ring.ord == :degrevlex
    @assert ring.ch > 0
    @assert length(gbexps) == length(gbcoeffs)

    @info "converting from $(ring.ord) to :lex with fglm"
    ...
    #  orderless
    staircase = Vector{Int}()

    #  lex
    nextmonomials = similar(staircase, 1)
