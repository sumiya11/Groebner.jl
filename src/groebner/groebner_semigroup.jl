...
function groebner_semigroup(polynomials, varmap, relations, groups, options)
    ...
    ring2, monoms2, coeffs2, options2 = io_convert_polynomials_to_ir(relations, deepcopy(_options))
    # TODO: multihom
    ((ring3, monoms3, coeffs3, _), (ring4, monoms4, coeffs4, _)) = map(group -> io_convert_polynomials_to_ir(group, deepcopy(_options)), groups)
    ring5, monoms5, coeffs5, options5 = io_convert_polynomials_to_ir(varmap, deepcopy(_options))
    params = AlgorithmParameters(ring, options)
    params.linalg =  LinearAlgebra(:deterministic, :sparse)
    params.reduced = false
    params.representation = PolynomialRepresentation(ExponentVector{UInt32}, params.representation.coefftype, params.representation.using_wide_type_for_coeffs)
    @log :info "" ring ring2 ring3 ring4 ring5
    @assert ring == ring2 == ring3 == ring4 == ring5
    gb_monoms, gb_coeffs = __groebner_semigroup1(ring, monoms, coeffs, params, monoms2, coeffs2, options2, monoms3, coeffs3, monoms4, coeffs4, monoms5, coeffs5)
    result = io_convert_ir_to_polynomials(ring, polynomials, gb_monoms, gb_coeffs, options)
    result
end

function __groebner_semigroup1(
    ring, monoms, coeffs,     params::AlgorithmParameters,
        monoms2, coeffs2, options2, monoms3, coeffs3, monoms4, coeffs4, monoms5, coeffs5
    ...
    gb_monoms, gb_coeffs = _groebner_semigroup2(ring, vcat(monoms, monoms2), vcat(coeffs, coeffs2), params)
