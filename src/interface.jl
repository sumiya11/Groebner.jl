...
- `linalg`: A symbol, linear algebra backend. Available options are: 
    ...
    - `:las_vegas` for probabilistic sparse linear algebra followed by a
      deterministic Groebner basis check, retrying F4 if the check fails. This
      option is available over finite prime fields.
...
- `monoms`: Monomial representation used in the computations. Available options
  are: 
    - `:auto` for the automatic choice, 
    - `:dense` for classic dense exponent vectors (default),
    - `:packed` for packed representation,
    - `:nibble` for nibble representation (4 bits per exponent). 
