...
- `monoms`: Monomial representation used during the computations. The algorithm
    automatically tries to choose the most suitable monomial representation. Otherwise,
...
- `seed`: The seed for randomization. Default value is `42`.
    The random number generator used in Groebner is `Random.Xoshiro`.
- `loglevel`: Logging level, an integer. Higher values mean less output from the program.
    Default value is `0`, which means that only warnings are printed.
