...
function groebner_apply!(trace, ring::PolyRing, monoms::AbstractVector, coeffs::AbstractVector; options...)
    keywords = KeywordArguments(:groebner_apply!, options)

    logging_setup(keywords)
    statistics_setup(keywords)

    trace = get_trace!(trace, ring.ch, keywords)

    io_extract_coeffs_raw_X!(trace, trace.representation, coeffs, ring.ch, keywords)
    ...
    result =
        groebner_apply1!(ring, trace, params)
