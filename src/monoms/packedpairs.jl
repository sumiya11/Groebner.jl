...
# Define PackedPairI types.

# PackedPairI{T, B} admits the interface of a vector
# of small nonegative integers with O(1) vector sum. 
# 
# PackedPairI{T, B} is implemented 
# as a sequence of I integer numbers,
# each integer {T} packs a fixed amount
...
function Base.hash(x::PackedPair3{T, B}, b::Vector{MH}) where {T, B, MH}
    ...
    h = h + packeddot(x.a2, b, B, 0)
    h = h + packeddot(x.a1, b, B, epc - max(epc - 1, length(b) - 2*epc))
    mod(h, MonomHash)
end
...
function monom_divmask(
    ...
    # @assert ndivvars <= 2*div(sizeof(T), sizeof(B)) - 1
