...
@testset "Custom multiplication by monomial" begin

    R, (x, y, z) = PolynomialRing(QQ, ["x", "y", "z"])

    ms = [R(1), x, 5x, x^2, x*y, 4x*z, (1//3)y^2, y*z, z^2,
            3x*y*z, x^2*y^3*z^4]
    Hs = [R(1) + x, x, x^2 + z^2*y^2, (1//8)x*z + 5, z^2*y - z*y, x*y*z,
          4x*y^2*z + x, 3x*y*z, x*y*z, 3x*y - 4]

    for m in ms
        for H in Hs
            true_prod = m * H
            my_prod   = GroebnerBases.multiplie_by_term(m, H)

            @test true_prod == my_prod
        end
    end
